...
// scripts/vra_vxs_direct_http_e2e_000089V4.ts
...
function readinessCommand(args, context) {
  const json2 = parseJsonOption(args, "vxs readiness [--json]");
  if (typeof json2 !== "boolean") return json2;
  const snapshot2 = buildVxsAgentContextSnapshot(context);
  const value = assessVxsReadiness(snapshot2);
  return {
    kind: "immediate",
    output: json2 ? `${JSON.stringify(value, null, 2)}
...
function recommendCommand(args, context) {
  const json2 = parseJsonOption(args, "vxs recommend [--json]");
  if (typeof json2 !== "boolean") return json2;
  const snapshot2 = buildVxsAgentContextSnapshot(context);
...
function appendApprovalEmitter(verificationCommand, packet) {
  const json2 = JSON.stringify(packet);
  if (process.platform === "win32") {
    return [
      verificationCommand,
      `Write-Output 'VXS_APPROVAL_POINT/1'`,
      `Write-Output ${powerShellLiteral(json2)}`,
      `Write-Output '[/VXS_APPROVAL_POINT/1]'`
    ].join("; ");
  }
  return [
    verificationCommand,
    `printf '%s\\n' 'VXS_APPROVAL_POINT/1' ${posixLiteral(json2)} '[/VXS_APPROVAL_POINT/1]'`
  ].join(" && ");
}
...
`;
function buildVxsPowerShellEnginePlan(request) {
  ...
  return {
    ...
    routeSummary: `VXS PowerShell Engine Provider: ${request.action} via System.Management.Automation metadata/AST`
  };
}
...
function engineExecution(request, context) {
...
function sha256File(path16) {
  return (0, import_crypto.createHash)("sha256").update((0, import_fs.readFileSync)(path16)).digest("hex");
}
...
function mapInvocation(request) {
...
function planVxsProviderExecution(projectRoot, request, powerShellProgram, powerShellCommand) {
...
var VertexShellService = class {
  ...
  async execute(request, sink) {
    ...
    const cwd = request.cwd?.trim() ? safeExistingDirectory(request.cwd.trim()) : this.cwd;
    this.cwd = cwd;
...
var VERA_VXS_REQUEST_SCHEMA = "vertex-vxs/vera-request-1";
var VERA_VXS_RESULT_SCHEMA = "vertex-vxs/vera-result-1";

// src/main/shell/vxs/vera-vxs-interface.ts
var MAX_COMMAND_CHARS2 = 32e3;
var MAX_RETURN_STREAM_BYTES = 512 * 1024;
...
// src/main/shell/vxs/vera-vxs-vra-direct-http-nerve.ts
var VERA_VXS_VRA_DIRECT_NERVE_SCHEMA = "vertex-vxs/vra-direct-request-1";
var VERA_VXS_VRA_DIRECT_RESPONSE_SCHEMA = "vertex-vxs/vra-direct-response-1";
var VERA_VXS_VRA_DIRECT_DEFAULT_HOST = "127.0.0.1";
var VERA_VXS_VRA_DIRECT_DEFAULT_PORT = 47834;
var MAX_BODY_BYTES = 64 * 1024;
var MAX_COMMAND_CHARS3 = 16 * 1024;
var MAX_CWD_CHARS = 4096;
var ORIGIN_RE = /^VERA(\d{2})$/;
var SESSION_RE = /^vera-(\d{2})$/;
...
function errorPayload(code, message, requestId = null) {
  return {
...
function validateOrigin2(origin) {
...
function validateRequest(value) {
  ...
  const cwd = input.cwd == null ? void 0 : String(input.cwd);
  ...
  if (!command.trim() || command.length > MAX_COMMAND_CHARS3) {
...
function createVeraVxsVraDirectServer(service, options = {}) {
    ...
    server.listen(port, host, () => {
      ...
      resolve9({
        ...
        close: () => new Promise((closeResolve, closeReject) => {
          server.close((error2) => error2 ? closeReject(error2) : closeResolve());
        })
      });
    });
  });
...
// scripts/vra_vxs_direct_http_e2e_000089V4.ts
async function post(port, command, requestId) {
...
async function main() {
  ...
  if (locked.status !== 423 || locked.body?.error !== "VERA_VXS_HUMAN_GATE_REQUIRED") {
  ...
  if (version.status !== 200 || version.body?.ok !== true || version.body?.result?.shellResult?.exitCode !== 0) {
  ...
  if (powershell.status !== 200 || powershell.body?.ok !== true || powershell.body?.result?.shellResult?.exitCode !== 0) {
  ...
  if (relocked.status !== 423 || relocked.body?.error !== "VERA_VXS_HUMAN_GATE_REQUIRED") {
