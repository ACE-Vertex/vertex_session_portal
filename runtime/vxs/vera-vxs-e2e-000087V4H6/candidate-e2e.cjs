...
function validateOrigin(origin2) {
  if (!origin2 || typeof origin2 !== "object") {
    throw new Error("VERA_VXS_INVALID_ORIGIN");
  }
  const vera = requiredText(origin2.vera, "origin_vera", 32).toUpperCase();
  const session = requiredText(origin2.session, "origin_session", 32).toLowerCase();
  const window = requiredText(origin2.window, "origin_window", 32).toLowerCase();
...
function normalizeRequest(input) {
  ...
  const origin2 = validateOrigin(input.origin);
...
// scripts/vera04_vxs_candidate_e2e_000087V4H6.ts
...
function classify(portal, registry, evidence) {
  ...
  const state = (stringValue(registry, "state") || stringValue(asRecord(registry?.record), "state") || stringValue(portal, "workstation_job_state")).toUpperCase();
...
function triageCommand(args, _context) {
  ...
  const state = stringValue(registry, "state") || stringValue(asRecord(registry?.record), "state") || stringValue(portal, "workstation_job_state");
  const allocated = stringValue(registry, "allocated_lane") || stringValue(asRecord(registry?.record), "execution_lane");
...
function renderText(snapshot) {
  const rows = [
    ...
    `  Frameworks: ${snapshot.workspace.frameworks.length ? snapshot.workspace.frameworks.join(", ") : "none detected"}`,
    "",
    "Git:",
...
function contextCommand(args, context) {
  ...
  return {
    kind: "immediate",
    output: option === "--json" ? `${JSON.stringify(snapshot, null, 2)}
` : renderText(snapshot),
    exitCode: 0,
    stream: "system"
  };
...
function assessVxsReadiness(snapshot) {
  const signals = [];
  let score = 100;
  ...
  const unavailableTools = snapshot.tools.filter((tool) => tool.version === null).map((tool) => tool.name);
...
function recommendVxsNextCommands(snapshot, readiness) {
...
function buildPlan2(context, commands) {
  ...
  const workspace = detectVxsWorkspace(context.cwd);
  const verifyMode = snapshot.workspace.git ? "changed" : "quick";
...
`,
      ...
      child.once("close", (code) => {
        const snapshot = this.active?.commandId === commandId ? this.active : active;
        const result = {
          commandId,
          command: redactCommand(command),
          cwd: executionCwd,
...
// src/main/ipc/register-vertex-shell-ipc.h6candidate.ts
...
async function main() {
  ...
  await win.loadURL("data:text/html,<html><body>VERA04 VXS H6 E2E</body></html>");
  const preloadReady = await win.webContents.executeJavaScript(
    `Boolean(window.veraVxs && typeof window.veraVxs.execute === 'function')`,
    true
  );
  if (!preloadReady) throw new Error("VERA04_VXS_PRELOAD_API_NOT_EXPOSED");
  const versionReq = request("vxs --version", "vera04-vxs-version-000087V4H6-candidate");
  const versionResult = await win.webContents.executeJavaScript(
    `window.veraVxs.execute(${JSON.stringify(versionReq)})`,
    true
  );
  const psReq = request("vxs ps engine", "vera04-vxs-ps-engine-000087V4H6-candidate");
  const psResult = await win.webContents.executeJavaScript(
    `window.veraVxs.execute(${JSON.stringify(psReq)})`,
    true
  );
  const assertCommon = (result, expectedId) => {
    if (result?.schema !== "vertex-vxs/vera-result-1") throw new Error("RESULT_SCHEMA_MISMATCH");
    if (result?.requestId !== expectedId) throw new Error("REQUEST_ID_MISMATCH");
    if (result?.origin?.vera !== "VERA04" || result?.origin?.session !== "vera-04" || result?.origin?.window !== "vera-04") throw new Error("ORIGIN_MISMATCH");
    if (result?.transport !== "VERTEX_SHELL_SERVICE") throw new Error("TRANSPORT_MISMATCH");
    if (result?.authorityEnforcement !== "VXS_EXISTING_POLICY") throw new Error("AUTHORITY_MISMATCH");
    if (result?.shellResult?.exitCode !== 0) throw new Error("EXIT_CODE_" + String(result?.shellResult?.exitCode));
  };
  assertCommon(versionResult, versionReq.requestId);
  assertCommon(psResult, psReq.requestId);
  if (versionResult.shellResult.backend !== "VXS_META") {
    throw new Error("VERSION_BACKEND_" + String(versionResult.shellResult.backend));
  }
  const versionText = Array.isArray(versionResult?.streamEvents) ? versionResult.streamEvents.map((event) => String(event?.chunk ?? "")).join("") : "";
  if (!versionText.includes("VXS 0.1.0")) throw new Error("VERSION_TEXT_MISSING");
  const badReq = request("Get-Process", "vera04-vxs-negative-000087V4H6-candidate");
  const negative = await win.webContents.executeJavaScript(
    `(async () => {
      try {
        await window.veraVxs.execute(${JSON.stringify(badReq)})
  ...
  if (!String(negative).includes("VERA_VXS_CANONICAL_VXS_COMMAND_REQUIRED")) {
    throw new Error("NON_VXS_GUARD_FAILED:" + String(negative));
  }
  console.log("VERA04_VXS_H6_CANDIDATE_E2E=PASS");
  console.log("ORIGIN=VERA04/vera-04/vera-04");
  console.log("PRELOAD_API=window.veraVxs.execute");
  console.log("IPC_CHANNEL=vera-vxs:execute");
  console.log("TRANSPORT=VERTEX_SHELL_SERVICE");
  console.log("VERSION_BACKEND=" + versionResult.shellResult.backend);
  console.log("VERSION_EXIT_CODE=" + versionResult.shellResult.exitCode);
  console.log("PS_ENGINE_BACKEND=" + String(psResult.shellResult.backend));
  console.log("PS_ENGINE_EXIT_CODE=" + psResult.shellResult.exitCode);
  console.log("NON_VXS_COMMAND_REJECTED=true");
  if (!win.isDestroyed()) win.destroy();
  import_electron5.app.quit();
}
main().catch((error2) => {
  console.error(error2 instanceof Error ? error2.stack ?? error2.message : String(error2));
  import_electron5.app.exit(71);
});
