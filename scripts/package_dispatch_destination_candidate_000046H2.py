...
TEMP = ROOT / "scripts/_build_dispatch_destination_candidate_000046H2.py"
...
def main():
    print("=== VERTEX SESSION PORTAL / 000046H2 CANDIDATE PACKAGE ===")
    if not BASE.exists():
        raise RuntimeError("000044_BUILDER_MISSING")

    source = BASE.read_text(encoding="utf-8")
    old_root = 'ROOT / "release" / "current" / "VertexSessionPortal-current-win-x64"'
    new_root = 'ROOT / "release" / "candidates" / "000046H2" / "VertexSessionPortal-000046H2-win-x64"'
    old_smoke = 'ROOT / "release" / "_smoke" / "session-portal-current"'
    new_smoke = 'ROOT / "release" / "_smoke" / "session-portal-000046H2"'
    ...
    exe = ROOT / "release/candidates/000046H2/VertexSessionPortal-000046H2-win-x64/Vertex Session Portal.exe"
    if not exe.exists():
