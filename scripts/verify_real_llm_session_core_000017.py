...
def main():
    print("VERTEX SESSION PORTAL / REAL LLM SESSION CORE VERIFY 000017")
    print(f"ROOT={ROOT}")
    ...
    if build.returncode != 0:
        print("BUILD=FAIL")
        print("VERTEX_SESSION_PORTAL_REAL_LLM_VERIFY_000017=FAIL")
        return 2

    print("BUILD=PASS")

    env = os.environ.copy()
    env["VERTEX_REAL_LLM_PROBE"] = "1"
    ...
    expected = [
        ...
        "VERTEX_SESSION_PORTAL_REAL_LLM_PROBE_000017=PASS",
    ]
    ...
    evidence = ROOT / "EVIDENCE" / "REAL_LLM_PROBE_000017"
    screenshot = evidence / "real-llm-three-vera.png"
    state_json = evidence / "real-llm-probe.json"

    if not screenshot.exists() or screenshot.stat().st_size == 0:
        print("REAL_LLM_SCREENSHOT_FILE=FAIL")
        print("VERTEX_SESSION_PORTAL_REAL_LLM_VERIFY_000017=FAIL")
        return 5

    if not state_json.exists() or state_json.stat().st_size == 0:
        print("REAL_LLM_JSON_FILE=FAIL")
        print("VERTEX_SESSION_PORTAL_REAL_LLM_VERIFY_000017=FAIL")
        return 6

    print(f"REAL_LLM_SCREENSHOT_EVIDENCE={screenshot}")
    print(f"REAL_LLM_JSON_EVIDENCE={state_json}")
    print("REAL_LLM_RUNTIME=PASS")
    print("VERTEX_SESSION_PORTAL_REAL_LLM_VERIFY_000017=PASS")
    return 0
