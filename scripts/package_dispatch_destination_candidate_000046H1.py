...
TEMP = ROOT / "scripts/_build_dispatch_destination_candidate_000046H1.py"
...
def main():
    print("=== VERTEX SESSION PORTAL / DISPATCH DESTINATION CANDIDATE PACKAGE 000046H1 ===")
    if not BASE.exists():
        raise RuntimeError("000044_BUILDER_MISSING")

    source = BASE.read_text(encoding="utf-8")
    old_root = 'ROOT / "release" / "current" / "VertexSessionPortal-current-win-x64"'
    new_root = 'ROOT / "release" / "candidates" / "000046H1" / "VertexSessionPortal-000046H1-win-x64"'
    old_smoke = 'ROOT / "release" / "_smoke" / "session-portal-current"'
    new_smoke = 'ROOT / "release" / "_smoke" / "session-portal-000046H1"'
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"CANDIDATE_PACKAGE_FAILED:{result.returncode}")
    finally:
        TEMP.unlink(missing_ok=True)

    exe = ROOT / "release/candidates/000046H1/VertexSessionPortal-000046H1-win-x64/Vertex Session Portal.exe"
    if not exe.exists():
        raise RuntimeError("CANDIDATE_EXE_MISSING")

    print(f"CANDIDATE_EXE={exe}")
    print("CANDIDATE_PACKAGE=PASS")
