...
ER=ROOT/"EVIDENCE"/"CORRELATION_INCIDENT_RUNTIME_SMOKE_000079V4C"
def ck(k,v):
    print(f"{k}={'PASS' if v else 'FAIL'}"); return bool(v)
def latest():
    xs=list(ER.rglob("correlation_incident_runtime_smoke_000079V4C.json")) if ER.exists() else []
...
def main():
    ...
    print(f"EVIDENCE={rp}")
    print("CORRELATION_INCIDENT_RUNTIME_SMOKE_000079V4C_VERIFY="+("PASS" if ok else "FAIL"))
