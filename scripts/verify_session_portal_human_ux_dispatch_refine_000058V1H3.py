...
def run(cmd, timeout=1800):
    ...
    if cp.stdout:
        out("STDOUT_TAIL=" + cp.stdout[-10000:].replace("\n", " | "))
    if cp.stderr:
...
# Exact compatibility contract that 000057V1 verifier requires.
...
# Safety service from H2 must remain intact.
...
# 32.5s equivalent polling model: 13 * 2.5s.
identity = object()
scroll = 191
expanded = True
status = "REGISTERED"
...
for tick in range(13):
    ...
    remount += int(identity is not before_identity)
    jump += int(scroll != before_scroll)
    reset += int(expanded != before_expanded)

    if status == "EXECUTING" and allocated == "lane-11":
        status_updated = True

checks["CARD_REMOUNT_ON_UNCHANGED_DATA=0"] = remount == 0
checks["SCROLL_JUMP=0"] = jump == 0
checks["EXPANSION_RESET=0"] = reset == 0
...
# Re-run the exact previously failing old Safety verifier.
...
out("=== VERTEX SESSION PORTAL / 057 SAFETY COMPAT REPAIR 000058V1H3 ===")
...
out("VERTEX_SESSION_PORTAL_HUMAN_UX_DISPATCH_REFINE_000058V1H3=" + ("PASS" if not failed else "FAIL"))
if failed:
