...
ER=PROJECT/"EVIDENCE"/"CORRELATION_INCIDENT_RUNTIME_SMOKE_000079V4C"

def stamp(): return dt.datetime.now().strftime("%Y%m%d-%H%M%S-%f")

def read_jsonl(path:Path,max_bytes=8*1024*1024):
...
def main():
    print("=== VERTEX SESSION PORTAL / CORRELATION + INCIDENT RUNTIME SMOKE 000079V4C ===")
    ...
    corr_records=[r for r in black if isinstance(r.get("correlation_id"),str) and r.get("correlation_id") and str(r.get("event","")).startswith(("vra_","human_","incoming_","workstation_","dispatch_","evidence_"))]
    ...
    marker_incident_black=[r for r in black if r.get("event")=="event_ray_incident_candidate" and isinstance(r.get("payload"),dict) and r["payload"].get("candidate_type")=="page:human_incident_marker"]
    ...
    if runtime_active and human_marker_e2e:
        ...
        classification="RUNTIME_ACTIVATION_INCOMPLETE"
        next_step="000079V4D_RUNTIME_ACTIVATION_DIAGNOSIS"

    run_dir=ER/stamp(); run_dir.mkdir(parents=True,exist_ok=True)
    rp=run_dir/"correlation_incident_runtime_smoke_000079V4C.json"
    report={
      "schema":"vertex-session-portal/correlation-incident-runtime-smoke/1",
    ...
    print(f"EVIDENCE={rp}")
    print("CORRELATION_INCIDENT_RUNTIME_SMOKE_000079V4C_RUN=PASS")
    return 0
