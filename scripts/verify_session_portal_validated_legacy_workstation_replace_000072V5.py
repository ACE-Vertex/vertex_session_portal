...
checks = {
    ...
    "NO_RENDERER_PID_OR_PATH_INPUT":
        "replaceValidatedLegacyListener(" in controller and
        "data-mode=\"replace-legacy\"" in lane,
    ...
    "SPINNER_ONLY_WHILE_BUSY":
        'data-mode="${this.workstationRuntimeMismatch() ? \'replace-legacy\' : \'start\'}"' in lane and
        '.startWorkstation[data-mode="replace-legacy"]:not(:disabled)' in css,
...
emit("=== SESSION PORTAL / VALIDATED LEGACY WORKSTATION REPLACE 000072V5 ===")
emit("OBSERVED_UI=WORKSTATION_LEGACY_RESTART_REQUIRED")
emit("SPINNING_HOVER_CAUSE=DISABLED_BUTTON_CURSOR_WAIT")
emit("REPLACEMENT=HUMAN_CLICK_ONLY")
emit("BLIND_KILL=ZERO")
emit("PORT=127.0.0.1:47832")
emit("POST_REPLACE_RUNTIME=CURRENT_SOURCE_CARGO")
emit("PORTAL_WORKSTATION_PROCESS_MERGE=ZERO")
for k, v in checks.items():
    emit(f"{k}={'PASS' if v else 'FAIL'}")
failed = [k for k, v in checks.items() if not v]
emit("VERTEX_SESSION_PORTAL_VALIDATED_LEGACY_WORKSTATION_REPLACE_000072V5=" + ("PASS" if not failed else "FAIL"))
if failed:
