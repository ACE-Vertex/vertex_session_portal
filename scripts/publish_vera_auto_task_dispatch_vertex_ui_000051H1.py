...
BASELINE = WORKSTATION / "SESSION_PORTAL_BUILDS" / "000050H1"
BUILDS = WORKSTATION / "SESSION_PORTAL_BUILDS"
STAGING = BUILDS / "000051H1.staging"
PUBLISHED = BUILDS / "000051H1"
LAUNCHER = WORKSTATION / "START_SESSION_PORTAL_LATEST.cmd"
POINTER = WORKSTATION / "SESSION_PORTAL_LATEST.pointer.json"
SMOKE = WORKSTATION / "_smoke_session_portal_000051H1"
...
def require_source():
    ...
    checks = {
        ...
        "VERTEX_DIALOG": "VERTEX // TASK DISPATCH" in bridge and "vertexDialog" in bridge,
...
def copy_runtime():
    if not (BASELINE / EXE_NAME).exists():
        raise RuntimeError("BASELINE_000050H1_MISSING")
...
def publish():
    ...
    launcher_text = (
        "@echo off\r\n"
        "setlocal\r\n"
        f'start "" "%~dp0SESSION_PORTAL_BUILDS\\000051H1\\{EXE_NAME}"\r\n'
    )
...
def main():
    print("=== VERTEX AUTO TASK DISPATCH + VERTEX UI PUBLISH 000051H1 ===")
