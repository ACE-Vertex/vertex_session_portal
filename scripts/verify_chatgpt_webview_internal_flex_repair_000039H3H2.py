...
def main():
    print('VERTEX SESSION PORTAL / CHATGPT WEBVIEW INTERNAL FLEX REPAIR VERIFY 000039H3H2')
    print('ROOT='+str(ROOT))
    ...
    flex_fill_ok=bool(re.search(r"flex\s*:\s*1\s+1\s+auto\s*;", b))
    checks.append(result('WEBVIEW_FLEX_FILL_PRESERVED', flex_fill_ok))
    ...
    # The original H3 verifier false-negative was the failure to recognize a valid
    # flex declaration. If the whitespace-tolerant matcher above recognizes the
    # live declaration, that false-negative path is retired. Avoid self-inspecting
    # the verifier text, which caused H3H1 to fail on its own diagnostic literal.
    checks.append(result('VERIFIER_FALSE_NEGATIVE_RETIRED', flex_fill_ok))
    if not all(checks):
        print('VERTEX_SESSION_PORTAL_CHATGPT_WEBVIEW_INTERNAL_FLEX_REPAIR_000039H3H2=FAIL')
        raise SystemExit(1)
    ...
    print('BUILD=PASS')
    print('VERTEX_SESSION_PORTAL_CHATGPT_WEBVIEW_INTERNAL_FLEX_REPAIR_000039H3H2=PASS')
